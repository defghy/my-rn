...
export default function getAbstract(
  ...
  function wrapStr(str) {
    ...
    if (SPECIAL_VAL.includes(str) || str.startsWith('Array[')) {
  ...
  if (circular) {
    ...
    json = obj + ''
    if (json.endsWith('Infinity') || json === 'NaN') {
      json = `"${json}"`
    } else {
      json = wrapNum(json)
    }
...
function canBeProto(obj) {
  const emptyObj = !!Object.getOwnPropertyNames(obj).length
